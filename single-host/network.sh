...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Showroom Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-showroom.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.manufacturer.example.com manufacturer
    peer_cert admin Admin@manufacturer.example.com manufacturer

    infoln "Creating Showroom Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.showroom.example.com showroom
    peer_cert admin Admin@showroom.example.com showroom
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while [[ $rc -ne 0 && $COUNTER -lt $MAX_RETRY ]]; do
      sleep 1
      set -x
      fabric-ca-client getcainfo -u https://admin:adminpw@localhost:7054 --caname ca-manufacturer --tls.certfiles "${PWD}/organizations/fabric-ca/manufacturer/ca-cert.pem"
      res=$?
    { set +x; } 2>/dev/null
    rc=$res  # Update rc
    COUNTER=$((COUNTER + 1))
  ...
  infoln "Generating CCP files for Manufacturer and Showroom"
...
# call the script to create the channel, join the peers of manufacturer and showroom,
...
function networkDown() {
  ...
  # stop customer containers also in addition to manufacturer and showroom, in case we were running sample to add customer
  COMPOSE_CUSTOMER_BASE_FILES="-f addCustomer/compose/${COMPOSE_FILE_CUSTOMER_BASE} -f addCustomer/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_CUSTOMER_BASE}"
  COMPOSE_CUSTOMER_COUCH_FILES="-f addCustomer/compose/${COMPOSE_FILE_CUSTOMER_COUCH} -f addCustomer/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_CUSTOMER_COUCH}"
  COMPOSE_CUSTOMER_CA_FILES="-f addCustomer/compose/${COMPOSE_FILE_CUSTOMER_CA} -f addCustomer/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_CUSTOMER_CA}"
  COMPOSE_CUSTOMER_FILES="${COMPOSE_CUSTOMER_BASE_FILES} ${COMPOSE_CUSTOMER_COUCH_FILES} ${COMPOSE_CUSTOMER_CA_FILES}"

  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_CUSTOMER_FILES} down --volumes --remove-orphans
  elif [ "${CONTAINER_CLI}" == "podman" ]; then
    ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_CUSTOMER_FILES} down --volumes
  else
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.manufacturer.example.com docker_peer0.showroom.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/showroom/msp organizations/fabric-ca/showroom/tls-cert.pem organizations/fabric-ca/showroom/ca-cert.pem organizations/fabric-ca/showroom/IssuerPublicKey organizations/fabric-ca/showroom/IssuerRevocationPublicKey organizations/fabric-ca/showroom/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/ordererOrg/msp organizations/fabric-ca/ordererOrg/tls-cert.pem organizations/fabric-ca/ordererOrg/ca-cert.pem organizations/fabric-ca/ordererOrg/IssuerPublicKey organizations/fabric-ca/ordererOrg/IssuerRevocationPublicKey organizations/fabric-ca/ordererOrg/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addCustomer/fabric-ca/customer/msp addCustomer/fabric-ca/customer/tls-cert.pem addCustomer/fabric-ca/customer/ca-cert.pem addCustomer/fabric-ca/customer/IssuerPublicKey addCustomer/fabric-ca/customer/IssuerRevocationPublicKey addCustomer/fabric-ca/customer/fabric-ca-server.db'
...
COMPOSE_FILE_CUSTOMER_BASE=compose-customer.yaml
# use this as the docker compose couch file for customer
COMPOSE_FILE_CUSTOMER_COUCH=compose-couch-customer.yaml
# certificate authorities compose file
COMPOSE_FILE_CUSTOMER_CA=compose-ca-customer.yaml
#
