...
# Reuse CLI delay if provided by network.sh, else default to 3 seconds.
DELAY=${DELAY:-${CLI_DELAY:-3}}
...
chaincodeInvoke() {
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    set -x
    peer chaincode invoke -o localhost:7050 -C $CHANNEL -n ${CC_NAME} -c ${CC_INVOKE_CONSTRUCTOR} --tls --cafile $ORDERER_CA  --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_MANUFACTURER_CA --peerAddresses localhost:9051 --tlsRootCertFiles $PEER0_SHOWROOM_CA  >&log.txt
    res=$?
    { set +x; } 2>/dev/null
    let rc=$res
    COUNTER=$(expr $COUNTER + 1)
  ...
  cat log.txt
  if [ $rc -ne 0 ] && grep -q "already exists" log.txt; then
    successln "Invoke returned 'already exists'; treating as idempotent success on channel '$CHANNEL'"
    rc=0
  fi
  if test $rc -eq 0; then
